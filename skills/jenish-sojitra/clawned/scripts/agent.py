...
CLAWNED_SERVER = os.getenv("CLAWNED_SERVER", "https://api.clawned.dev")
...
def api_request(endpoint, data=None, method="POST"):
    if not CLAWNED_API_KEY:
        print("[!] CLAWNED_API_KEY not set. Get your key at https://clawned.dev/settings"); sys.exit(1)
...
SCANNABLE_EXTS = {".md", ".py", ".js", ".ts", ".sh", ".mjs", ".cjs", ".jsx", ".tsx",
    ...
    ".cfg", ".conf", ".lua", ".go", ".rs", ".r", ".ps1", ".bat", ".cmd", ".txt", ".env", ".ini"}
MAX_FILE_SIZE = 512 * 1024  # 512KB per file
MAX_FILES = 30

def collect_skill_files(path):
    """Read scannable files from a skill directory."""
    files = {}
...
def read_skill(name, path, source, dir_owner=""):
    """Read a skill directory. Prefers _meta.json for the new format, falls back to SKILL.md."""
    ...
    # Use directory name as displayName; truncate description if used as fallback
    display = name
    if desc:
...
def cmd_scan(path=None, slug=None):
    if path:
        ...
        try:
            ...
            s = read_skill(os.path.basename(path), path, "manual")
            if s:
