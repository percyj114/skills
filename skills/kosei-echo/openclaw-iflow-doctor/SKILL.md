...
```powershell
# 1. 验证文件存在
Test-Path "$env:USERPROFILE\.openclaw\skills\openclaw-iflow-doctor\openclaw_memory.py"

# 2. 验证 Python 可以运行
python "$env:USERPROFILE\.openclaw\skills\openclaw-iflow-doctor\openclaw_memory.py" --version
...
[Environment]::SetEnvironmentVariable(
    "PATH", 
    $env:PATH + ";$env:USERPROFILE\.openclaw\skills\openclaw-iflow-doctor",
    "User"
)
```
...
| enable_iflow_helper | true | 自动修复失败时是否调用 iflow-helper |
| similarity_threshold | 0.7 | 案例匹配相似度阈值 |
| max_bat_files | 2 | 最多生成 BAT 文件数 |
| language | auto | 报告语言（auto/zh/en） |
| iflow_memory.enabled | true | 是否启用 iFlow 记忆功能 |
| iflow_memory.save_repair_records | true | 是否将修复记录保存到 iFlow 记忆目录 |

### 关闭 BAT 文件生成

如果你不想生成 BAT 文件，只保留诊断报告：

```bash
# 编辑配置文件
# ~/.openclaw/skills/openclaw-iflow-doctor/config.json
...
# 禁用 iflow-helper 调用（纯本地模式）
openclaw skills config openclaw-iflow-doctor --set enable_iflow_helper=false
```

### 服务器无图形界面认证（SSH/无显示器）

如果服务器没有图形界面，OAuth 浏览器认证会失败：
...
```bash
# 步骤 1：在本地电脑浏览器访问 https://iflow.cn/
# 步骤 2：登录 → 设置 → API Key → 生成并复制

# 步骤 3：在服务器配置
export IFLOW_API_KEY="your-api-key-here"

# 永久生效（添加到 ~/.bashrc）
echo 'export IFLOW_API_KEY="your-api-key-here"' >> ~/.bashrc
source ~/.bashrc
